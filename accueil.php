...
*/
$req = $bdd->query("SELECT lm.vainqueur lp.gain lp.est_payer lp.equipe FROM les_matchs as lm  INNER JOIN les_paris as lp on lp.ID_match = lm.ID where lm.status_match = 2 AND lp.ID_utilisateur = " . $_SESSION["id"]);
    ...
    foreach($matchs_payer as $match_payer){
        if($match_payer["est_payer"] == 0){
            if($match_payer["vainqueur"] == $match_payer["equipe"]){
                $req = $bdd->prepare("UPDATE les_paris as lp SET lp.est_payer = 1, gain = gain + mise WHERE ID_match = :id_match");
                $req->execute([
                    "id_match" => $match_payer["ID_match"]
                ]);
            }else{
                $req = $bdd->prepare("UPDATE les_paris SET est_payer = 1 WHERE ID_match = :id_match");
                $req->execute([
                    "id_match" => $match_payer["ID_match"]
                ]);
            }
